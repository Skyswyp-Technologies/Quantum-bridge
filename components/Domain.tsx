...
const SuccessModal: React.FC<{ isOpen: boolean; onClose: () => void; domain: string }> = ({ isOpen, onClose, domain }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-[#1A1A1ACC] border border-[#A6A9B880] rounded-lg p-6 max-w-sm w-full mx-4">
        ...
        <p className="text-gray-300 mb-4">You have successfully claimed your domain!</p>
  ...
  return (
    <div className="bg-[#000000] text-white w-full min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex">
        {isMobile ? (
          <MobileDomainContent domain={domain} setDomain={setDomain} handleClaim={handleClaim} />
        ) : (
          <DesktopDomainContent domain={domain} setDomain={setDomain} handleClaim={handleClaim} />
        )}
      </div>
      <SuccessModal 
...
const MobileDomainContent: React.FC<ContentProps> = ({ domain, setDomain, handleClaim }) => (
    ...
    <div className="flex-1 flex flex-col rounded-3xl border border-[#3E4347] relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col justify-between p-4 z-10">
        <div className="w-full max-w-md mx-auto space-y-6">
          ...
          <h2 className="text-xl text-[#A6A9B8] font-semibold text-center">Discover and get your domain.</h2>
          
          <div className='w-full bg-[#1A1A1A80] border border-[#3E434773] rounded-lg p-4 flex flex-col gap-4 backdrop-blur-sm'>
            <label className='text-gray-400 text-sm'>Enter your preferred domain name</label>
...
const DesktopDomainContent: React.FC<ContentProps> = ({ domain, setDomain, handleClaim }) => (
    ...
    <div className="w-full max-w-3xl space-y-5 text-center relative z-10">
      ...
      <div className='w-full mx-auto bg-[#1A1A1A80] border border-[#3E434773] rounded-lg p-6 flex flex-col gap-8 backdrop-blur-sm'>
        <label className='text-[#FFFFFF] text-xl text-left'>Enter your preferred domain name</label>
